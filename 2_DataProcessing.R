...
# Loading all raw data: 15-20 min
...
# Removing abnormal values from site 24

rd_all %>% 
  filter(!(SiteID == 24 & 
             datetime > "2022-07-20 00:00:00" &
             datetime < "2022-07-20 23:59:59")) -> rd_all
...
cdf_builder <- function(siteid, variable){
  # siteid = 12
  # variable = 'dvol'
  labels_df = tibble(variable = c('d_lastirr', 'n_eve', 'dmin', 'fr', 'dvol'), 
                     label = c('Days since last irrigation event',
...
# example - use
cdf_builder(siteid = 48, variable = 'n_eve')
cdf_builder(siteid = 24, variable = 'd_lastirr')
cdf_builder(siteid = 66, variable = 'dvol')


# Analysis at the weekly level 
# Add the budget
kc = 0.8 # generalization - crop coefficient # probably better to use 1.05 instead of 0.8??
...
#          VolumeVsBudget = IrrigationVolume_in - budget,
#          over_budget = ifelse(VolumeVsBudget > 0,VolumeVsBudget,0),
#          under_budget = ifelse(VolumeVsBudget < 0, VolumeVsBudget, 0)) -> df_test_weekly       # actual difference between Volume and budget
